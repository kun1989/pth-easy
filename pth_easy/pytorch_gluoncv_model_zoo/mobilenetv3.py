...
class MobileNetV3(nn.Module):
    def __init__(self, cfg_stages, first_channels, cls_ch_squeeze, cls_ch_expand, classes=1000):
        ...
        # maybe a mistake has no bias without bn
        # self.last_conv2 = Conv2d_1x1(cls_ch_squeeze, cls_ch_expand, norm="", act="hard_swish")
        self.last_conv2 = nn.Conv2d(cls_ch_squeeze, cls_ch_expand, 1, 1, 0, bias=False)
        self.last_conv2_act = get_act("hard_swish")

        self.output = nn.Conv2d(cls_ch_expand, classes, kernel_size=1, stride=1, padding=0)
